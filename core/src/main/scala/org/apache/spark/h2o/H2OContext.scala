...
class H2OContext private (@transient val sparkContext: SparkContext, @transient conf: H2OConf) extends org.apache.spark.Logging
  ...
  def asDataFrame(fr : H2OFrame)(implicit sqlContext: SQLContext) : DataFrame = toDataFrame(this, fr)
  def asDataFrame(fr: Frame)(implicit  sqlContext: SQLContext) : DataFrame = toDataFrame(this, new H2OFrame(fr))
